...
log_error() {
    sed -i "1s/^/$(date '+%Y-%m-%d %H:%M:%S') - Error: Paur Site $1\n/" /var/log/generate_post_today.log
...
send_mail() {
    ...
    echo "Post generation failed today ($(date '+%Y-%m-%d')). Please find the attachment for further information." | mail -s "Paur Site | $1" -A ./log.txt vishwa0403@gmail.com soried@gmail.com 2>> /var/log/send_mail.log
...
run_command cp -r _site/* /var/www/panafricanupdates/public_html/
git add .
run_command git commit -m "post: $(date '+%Y-%m-%d')"
run_command git push origin main
echo "$(date '+%Y-%m-%d %H:%M:%S') - Paur Site Script executed successfully." >> /var/log/generate_post.log 
echo "----------------" >> /var/log/generate_post_today.log
exit 0
